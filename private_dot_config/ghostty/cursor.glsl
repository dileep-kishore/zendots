...
// Catppuccin Mocha colors (dimmed)
const vec4 TRAIL_COLOR = vec4(0.596, 0.451, 0.769, 0.25); // dimmed mauve
const vec4 CURRENT_CURSOR_COLOR = TRAIL_COLOR;
const vec4 PREVIOUS_CURSOR_COLOR = TRAIL_COLOR;
const vec4 TRAIL_COLOR_ACCENT = vec4(0.506, 0.545, 0.796, 0.18); // dimmed lavender
const float DURATION = .3; // Faster fade
const float OPACITY = .12; // Lower opacity
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (isFarEnough && isOnSeparateLine) {
        ...
        // Apply reduced opacity for gentler trail
        vec4 accentColor = TRAIL_COLOR_ACCENT * OPACITY;
        vec4 mainColor = TRAIL_COLOR * OPACITY;

        newColor = mix(newColor, accentColor, (1.0 - smoothstep(sdfTrail, -0.01, 0.001)) * 0.5);
        newColor = mix(newColor, mainColor, antialising(sdfTrail) * 0.6);
        newColor = mix(fragColor, newColor, (1.0 - alphaModifier) * 0.7);
